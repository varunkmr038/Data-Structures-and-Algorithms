...
int solve(vector<int>& prices, int i, bool buy, int& fee, vector<vector<int>>& dp) {
    ...
    if (buy == 1) {
        // buy cur stock
        int buyCurStock = solve(prices, i + 1, 0, fee, dp) - prices[i]; // if you are buying then adding - prices
        ans = max(ans, buyCurStock);
    }

    // you cannot buy since  you have the stock
    else {
        int sellLastStock = prices[i] - fee + solve(prices, i + 2, 1, fee, dp); // selling then adding  + prices
        ans = max(ans, sellLastStock);
    }

    // skip and do nothing
    ans = max(ans, solve(prices, i + 1, buy, fee, dp));
